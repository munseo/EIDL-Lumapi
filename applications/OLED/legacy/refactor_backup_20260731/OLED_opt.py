...
# =============================================================================
# OLED_opt.py  -  results-informed "optimal" OLED angular-emission optimizer
# =============================================================================
# Synthesized after analyzing the execution outputs of every prior attempt
# (OLED_lens / OLED_new forward-dipole runs, OLED / OLED_Min reciprocity runs in
# /home/eidl/Lumerical_data/{Done,Failed}). What the RESULT files showed:
#
#   - Forward/dipole (OLED_lens, newest Done run): the postprocess order-shares
#     put only 0.5% of the power at 0 deg (target ~14%) and DUMPED ~50% above
#     50 deg (target 0): 52.5deg=20.6%, 65.1deg=11.1%, ... The phi=0 (radial_1d)
#     FoM was blind to the off-axis (m,n) diffraction orders, so it never saw or
#     penalized that high-angle leakage.
#   - Reciprocity (OLED_Min): the FoM claimed a good 0:45 = 1:0.76 ratio, yet the
#     postprocess angular pattern PEAKED at ~51 deg. Constraining only 3 discrete
#     angles left every other direction free, and power escaped upward.
#   - Every OLED_new run sat in Failed/ - the GPU "layout mode" reload bug (fixed).
#
# Root cause (common to all): power leaks into UNCONSTRAINED high angles, and at
# period 2.5um the square lattice offers 13 propagating orders up to 79.7 deg, so
# there is nowhere for the design to hide the light except high angles.
#
# Fixes baked in here:
#   1. Period default 1.1um (was 2.5). At lambda=0.55um, lambda/P=0.5 so the ONLY
#      propagating orders are 0deg, 30deg (1,0), 45deg (1,1); everything >=2nd order
#      is evanescent. High-angle leakage is physically impossible, and the ramp
#      target's intermediate angle (30deg) is available. Period is THE dominant
#      lever: pick it so the desired orders land on the target and nothing sits
#      above the max target angle. (P = lambda/sin(theta) for the (1,0) order.)
#   2. FoM = kspace_2d ramp T*M (default): the full 2D monitor DFT captures every
#      (m,n) order; energy outside [0, max target angle] reduces throughput T and is
#      penalized. Fixes the radial_1d blindness that let 55/65 deg leak unseen.
#   3. GPU session results kept via a layoutmode-guarded reload (no more Failed/).
#   4. 2D DFT via two O(N^3) contractions (a single 3-operand einsum was O(N^4)).
#   5. Coherent even-spaced dipole grid samples OFF-axis, so the FoM sees the
#      high-angle emission that on-axis-only sampling missed.
#   6. Postprocess = incoherent single-dipole far-field with signed radiance,
#      per-dipole |E| images and a polarization sweep, for an honest opt-vs-PP check.
#
# The FoM itself (throughput * match**ratio_emphasis vs a linear angle ramp) is
# unchanged from OLED_new; this file is that machinery with results-informed
# defaults and the period lever made explicit.
# =============================================================================
...
# Period is the dominant angular lever. Default 1.1um -> propagating orders only at
# 0/30/45 deg (lambda/P=0.5), nothing above the 45-deg max target -> no high-angle
# leakage possible. Widen it only if the target needs orders above 45 deg.
window_x = env_float("MSOPT_OLED_PERIOD_X_UM", 1.1)
window_y = env_float("MSOPT_OLED_PERIOD_Y_UM", 1.1)
