...
def nearest_order_labels(ukx, uky, orders, propagating):
    """Power-conserving, non-overlapping nearest-order partition."""
    if not orders:
...
def n2f_spectrum(sim, monitor_name, wavelength_um=None, na=None):
    """Near-to-far power samples on a uniform direction-cosine grid.

    The returned spectrum includes dOmega=dux*duy/cos(theta), which is required
    for power-conserving angle and diffraction-order integration.
    """
...
def angular_radiance_value(theta_grid, spectrum, center_deg, half_width_deg=None):
    ...
    mask = (
        theta_grid <= half_width_deg
        if center_deg <= 1e-12
        else np.abs(theta_grid - center_deg) <= half_width_deg
    )
...
def central_cell_dipoles(n_samples, pol="x"):
    ...
    default_grid_n = int(round(np.sqrt(max(1, env_int("MSOPT_OLED_OPT_DIPOLE_COUNT", 25)))))
    grid_n = env_int("MSOPT_OLED_PP_DIPOLE_GRID", default_grid_n)
    if grid_n > 0:
        xs = (np.arange(grid_n, dtype=float) + 0.5) * active_x / grid_n - 0.5 * active_x
        ys = (np.arange(grid_n, dtype=float) + 0.5) * active_y / grid_n - 0.5 * active_y
...
def postprocess_polarizations():
    ...
    raw = os.environ.get(
        ...
        os.environ.get("MSOPT_OLED_POSTPROCESS_POLARIZATION", ",".join(opt_pols)),
    )
    pols = [p.strip().lower() for p in raw.replace(";", ",").replace(" ", ",").split(",") if p.strip()]
    for p in pols:
        if p not in ("x", "y", "z"):
...
def run_postprocess(final_design):
    ...
    if np.asarray(visible_wavelengths).size != 1:
        raise ValueError(
    ...
    pp_mode = os.environ.get("MSOPT_OLED_PP_MODE", "tile").strip().lower()
    ...
    for pol in pols:
        post_sources = central_cell_dipoles(n_dipoles, pol)
        for i, (x, y, z, _p) in enumerate(post_sources):
            ...
            try:
                ...
                raw_spectrum_sum = float(np.sum(np.maximum(spectrum, 0.0)))
                if not np.isfinite(raw_spectrum_sum) or raw_spectrum_sum <= 0.0:
                    raise RuntimeError("far-field spectrum has zero/non-finite propagating power")
                # Calibrate the FFT/n2f angular samples to the independently
                # measured absolute top-monitor power before incoherent summation.
                spectrum = np.maximum(spectrum, 0.0) * top_power_abs / raw_spectrum_sum
                ...
                per_dipole.append({
                    ...
                    "eff": ring_flux / max(total_emitted, channel_power_floor),
    ...
    valid_records = [
        ...
        if np.isfinite(rec[8]) and np.isfinite(rec[7]) and rec[7] > 0.0
    ]
    mean_lee = (
        float(np.sum([rec[8] for rec in valid_records]) / np.sum([rec[7] for rec in valid_records]))
        if valid_records
        else float("nan")
    )
    ...
    print(f"[postprocess] full ensemble LEE (sum extracted top / sum dipole power): {mean_lee:.6e}; individual min={min_lee:.6e}, max={max_lee:.6e} over {lee_values.size} dipoles")
    ...
    if len(pol_spectra) > 1:
        for pol, spec in pol_spectra.items():
            ...
            pol_records = [
                rec for rec in records
                if rec[4] == pol and np.isfinite(rec[8]) and np.isfinite(rec[7]) and rec[7] > 0.0
            ]
            pol_lee = (
                float(np.sum([rec[8] for rec in pol_records]) / np.sum([rec[7] for rec in pol_records]))
                if pol_records
                else float("nan")
            )
    ...
    print(f"[postprocess] integrated zero-order power={zero_order_power:.6e} W, share={zero_order_share * 100:.3f}%")
