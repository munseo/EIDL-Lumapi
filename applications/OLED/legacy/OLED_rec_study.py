...
W_AT_MAX = 0.9                # requested share at TARGET_MAX_ANGLE; the rungs in
...
# _sep_score below now only covers the suppress term
# (grouped_suppress_J calls _sep_score directly) -- the transmission term uses
# level_score's own bounded saturating-ratio form instead (see its comment for
# why). Both hold for the family
#
#       J_i = w_i * h(p_i / w_i),   h concave
#
# whose stationary condition h'(T) = lambda is the same for every i, hence p ∝ w.
#
# The score used is J_i = sqrt(w_i * p_i). Cauchy-Schwarz gives
# sum_i sqrt(w_i p_i) <= sqrt(sum w * sum p), so it is bounded and, more
# importantly, NON-NEGATIVE by construction -- which msopt's optimizer assumes
# (see _sep_score).
#
# REMOVED, and why, so neither comes back:
#   log  J_i = w_i * ln(p_i / eps + 1). Same ratio optimum in theory, but
#        w_i*ln(p_i) is negative for p < 1 and needs a shift by a constant that
#        depends on LOG_EPS -- so the FoM's zero point, and with it every
#        Armijo/best-so-far comparison, was an artifact of a numerical floor
#        rather than of the physics. A negative FoM also broke msopt outright:
#        Best[6] starts at 0, so the global best never updated, the geometry was
#        never saved and Warm_restarter's sign-flipped test escalated the
#        learning rate every third iteration. Not a tuning problem -- the shape
#        was wrong for the optimizer it feeds.
#   sum  J_i = w_i * p_i. Linear, so under the Parseval budget sum(p) <= 1 the
#        optimum is a VERTEX: it cannot express a ratio at all and collapses
#        1.00/0.93/0.85 to 1/0/0, which is the one thing this FoM exists to do.
...
#              match_J normalizes by sum_i(p_i) before scoring (see its own
#              docstring), so any common per-mode scale factor cancels out of
#              the shape score by construction -- "absolute" below would not
#              actually corrupt match_J the way it corrupted the old weighted-
#              sum design (T MULTIPLYING the whole shape score, so raising the
#              total was always cheaper than fixing the ratio -- seen in the
#              v8 run: T 3.4 -> 5.1 carried more of the FoM rise than the shape
#              did, first-order ratio went BACKWARDS 0.369 -> 0.178). It WOULD
#              still corrupt transmission_term, though: that already multiplies
#              by T itself (see its docstring), so "absolute" would double that
#              factor into the level score. Untested combination -- "purity"
#              is the only one actually exercised, keep it that way unless
#              transmission_term is revisited too.
#   "absolute" a_i = overlap / (N * planar reference) = p_i * T. Level and shape
#              in one number instead of kept separate.
...
#   1. Plain per-mode score_one summed across 7 adjoints. A
